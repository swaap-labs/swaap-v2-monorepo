...
contract WstETHToBasePriceCache is IWstETHToBasePriceAdapter {
  /**
  ...
  struct RateCache {
    ...
    uint128 lastRateUpdate;
  }

  RateCache public rateCache;
  ...
  function latestRoundData() external view override returns (uint80, int256, uint256, uint256, uint80) {
    ...
    RateCache storage _rateCache = rateCache;

    _srequire(stEthToBasePrice > 0 || _rateCache.latestCachedRate <= 0, SwaapV2Errors.NON_POSITIVE_PRICE);

    int256 scaledStEthToBasePrice = stEthToBasePrice * STETH_TO_BASE_SCALE_FACTOR;

    int256 wstEthToBasePrice = scaledStEthToBasePrice * _rateCache.latestCachedRate / int256(ONE);

    uint256 time = updatedAt < _rateCache.lastRateUpdate ? updatedAt : _rateCache.lastRateUpdate;
  ...
  function updateCache() public {
    ...
    require(rate < uint128(type(int128).max), "rate overflow");
    _rateCache.latestCachedRate = int128(uint128(rate));
    _rateCache.lastRateUpdate = uint128(block.timestamp);  
  }
